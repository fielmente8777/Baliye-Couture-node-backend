...
export async function createProduct(data: Partial<IProduct>, adminId?: string) {
  ...
  if (!data.garmentTypeId) throw ApiError.badRequest('A product needs a garment type');

  const garmentType = await garmentTypeRepository.findById(String(data.garmentTypeId));
  if (!garmentType) throw ApiError.badRequest('That garment type does not exist');
...
export async function updateProduct(id: string, data: Partial<IProduct>, adminId?: string) {
 ...
 */
export async function setStatus(id: string, status: IProduct['status'], adminId?: string) {
  const product = await productRepository.findById(id);
  if (!product) throw ApiError.notFound('Product not found');
  ...
  return productRepository.updateById(id, { status, updatedBy: adminId as never });
}
